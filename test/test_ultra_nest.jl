...
@testset "test_ultranest" begin
    ...
    loglikelihood = let dist = dist
        function (x::AbstractVector{<:Real})
            ll = logpdf(dist, x)
            # logpdf on MixtureModel returns NaN or -Inf in gaps between
            # distributions, and UltraNest doesn't like -Inf, so return -1E10
    ...
    X_un = pyconvert(Matrix{Float64}, py_result["samples"])
    ...
    logz = pyconvert(Float64, py_result["logz"])
    logzerr = pyconvert(Float64, py_result["logzerr"])
