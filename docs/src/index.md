...
This Julia package is currently just a very thin wrapper around the Python ultranest package, which is automatically installed via [CondaPkg.jl](https://github.com/JuliaPy/CondaPkg.jl).
...
Currently UltraNest.jl exports a single global constant `ultranest`. [PythonCall.jl](https://github.com/JuliaPy/PythonCall.jl) makes using ultranest from Julia very similar to the equivalent Python code.
...
See the [UltraNest Python documentation](https://johannesbuchner.github.io/UltraNest/) regarding usage.
...
`result` is a Python dict that contains the samples (in weighted and unweighted form), the logarithm of the posterior's integral and additional valuable information. Since our prior was flat, we can compare the samples generated by UltraNest with truly random (IID) samples drawn from `dist`:
